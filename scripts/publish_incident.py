...
def main() -> None:
    ...
    missing = [name for name, val in (("pair", args.pair), ("stage", args.stage), ("error", args.error)) if not val]
    if missing:
        parser.error(f"the following arguments are required: {', '.join(f'--{m}' for m in missing)}")
